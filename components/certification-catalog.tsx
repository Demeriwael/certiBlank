  ...
  return <>
    ...
    {status === "ready" && <>
      ...
      {sections.filter(platform => platform.ready.length || platform.upcoming.length).map(platform => <section className="track-section" key={platform.slug} aria-labelledby={`heading-${platform.slug}`} style={{ "--platform-accent": platform.accent } as CSSProperties}>
        <div className="track-section-heading"><div><PlatformLogo slug={platform.slug} /><h2 id={`heading-${platform.slug}`}>{platform.name}<span>{platform.category}</span></h2></div>{!platformSlug && <Link className="text-action" href={`/platforms/${platform.slug}`}>Explore platform<CatalogIcon /></Link>}</div>
