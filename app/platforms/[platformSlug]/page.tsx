...
export default async function PlatformHub({ params }: { params: Promise<{ platformSlug: string }> }) {
  ...
  return <div className="site-shell"><header className="site-header"><Brand /><Link className="nav-link" href="/certifications">All certifications <CatalogIcon /></Link><AccountLink /></header><main className="catalog-page section-wrap"><CertificationCatalog key={platformSlug} platformSlug={platformSlug} /></main></div>;
}
