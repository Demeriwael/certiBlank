      ...
      {loading ? <SetupSkeleton /> : <>
        ...
        {!setup?.config || !setup.available ? <div className="exam-card"><h2>Fresh questions are on the way.</h2><p>This certification needs an updated question bank before these practice modes become available.</p></div> : <>
          ...
          <section className="exam-card setup-controls"><div className="eyebrow section-kicker">{examMode === "domain" ? "MAKE IT YOURS" : "EXAM BRIEFING"}</div><h2>{examMode === "domain" ? "Where do you want to focus?" : "Ready for the full picture?"}</h2>
            {examMode === "domain" ? <><div className="domain-options">{setup.domains.map(d => <label key={d.id}><input type="checkbox" checked={domains.includes(d.id)} disabled={!d.available} onChange={e => setDomains(e.target.checked ? [...domains,d.id] : domains.filter(id => id !== d.id))}/><span>{d.name}<small>{d.available} questions available</small></span><b>{d.weightPercent}% of blueprint</b></label>)}</div></> : <><div className="mock-facts"><div><strong>{setup.config.questionCount}</strong><span>Blueprint-weighted questions</span></div><div><strong>{setup.config.durationSeconds/60} min</strong><span>Timer continues on refresh</span></div><div><strong>{setup.config.scoring.passingScore}</strong><span>Practice passing score</span></div></div><p>Answers and explanations appear after submission. Multiple-response questions require every correct option; no partial credit. Unanswered questions count as incorrect. Time expiry submits your saved answers automatically.</p><p className="scoring-note">The linear {setup.config.scoring.minimumScore}–{setup.config.scoring.maximumScore} score is a practice estimate, not the vendor’s official scoring model.</p>{!setup.mockReady && <p role="status">More questions are needed to fill the blueprint: {setup.domains.filter(d => d.available < d.required).map(d => `${d.name} (${d.available}/${d.required})`).join(", ")}.</p>}</>}
          </section></div>
          <div className="setup-actionbar">
            <div>{examMode === "domain" ? <label htmlFor="session-length">Session length<select id="session-length" value={limit} disabled={busy} onChange={e => setLimit(Number(e.target.value))}>{[5,10,20,30,50].map(n => <option key={n} value={n}>{n} questions</option>)}</select></label> : <p><strong>{setup.config.questionCount} questions</strong><span>{setup.config.durationSeconds / 60} minutes · Target {setup.config.scoring.passingScore}</span></p>}</div>
            <button className="primary-button" disabled={busy || (examMode === "domain" ? !available : !setup.mockReady)} aria-busy={busy} onClick={start}>{busy ? "Preparing…" : examMode === "domain" ? `Start ${Math.min(limit, available)} questions` : "Begin timed exam"}<ExamIcon name="next" /></button>
          </div></>}
