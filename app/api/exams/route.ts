...
export async function POST(request: Request) {
  try {
    ...
    if (body.mode === "mock") {
      ...
      if (!Array.isArray(body.domains) || !body.domains.length || body.domains.length > bank.domains.length || body.domains.some((id: unknown) => !bank.domains.some(d => d.id === id)) || typeof body.limit !== "number" || !Number.isInteger(body.limit) || body.limit < 1 || body.limit > 200) return Response.json({ error: "Select valid domains and 1–200 questions" }, { status: 400 });
      const domains = body.domains;
      questions = shuffled(bank.questions.filter(q => domains.includes(q.domain))).slice(0, body.limit);
    ...
    const response = requestError(error, "Unable to start exam");
    if (response.status === 500) console.error("POST /api/exams failed", databaseDiagnostics(error, process.env));
    return response;
  }
}
